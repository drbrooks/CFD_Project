...
resinit = [0,0,0];          % Initial iterative residual for each equation (from iteration 4)
...
for n = ninit:nmax
    ...
    [res, resinit, conv] = check_iterative_convergence(n, res, res2, resVect, resinit, ninit, rtime, dtmin);
...
%         beta2 = max(uvel2,rkappa*vel2ref);
%         
%         lambda_x = half*(abs(u(:,:,2))+sqrt(u(:,:,2).^2+four.*beta2));
%         lambda_y = half*(abs(u(:,:,3))+sqrt(u(:,:,3).^2+four.*beta2));
%         lambda_max = max(lambda_x,lambda_y);
%         
%         dtconv = dx./lambda_max; % Convective Time Step
%         
%         dt = cfl.*min(dtconv,dtvisc);
%         dtmin = min(min(min(dt)),dtmin);
...
for j = 2:jmax-1
    ...
    % Special Consideration for Boundaries
...
for i = 2:imax-1
    ...
    artviscy(i,jmax-1) = artviscy(i,jmax-2);
end
...
function [res, resinit, conv] = check_iterative_convergence...
    (n, res, res2, resVect, resinit, ninit, rtime, dtmin)
%
...
% 
% for k = 1:neq
%     res(k) = sqrt(sum(sum(abs(((u(i,j,k)-uold(i,j,k))./dt(i,j))).^2))/(imax*jmax));
% end
% 
% if(n==4)
%     resinit = res;
